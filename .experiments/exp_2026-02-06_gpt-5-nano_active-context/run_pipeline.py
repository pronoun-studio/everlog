...
2) summarize each event with gpt-5-nano, using non-active OCR as reference context
...
def _split_active_inactive(ocr_by_display: Any) -> tuple[str, list[str]]:
    active_text = ""
    inactive: list[str] = []
    ...
    for item in ocr_by_display:
        if not isinstance(item, dict):
            continue
        text = _collapse_ws(str(item.get("ocr_text") or ""))
        if not text:
            continue
...
def step_extract_active_ocr(date: str, *, run_id: str | None, prefer: str) -> Stage02Pick:
    ...
    for e in _read_jsonl(pick.path):
        total += 1
        active_text, inactive_texts = _split_active_inactive(e.get("ocr_by_display"))
        if active_text:
            nonempty += 1
        ...
        rows.append(
            {
                ...
                "inactive_display_ocr_texts": inactive_texts,
            }
        )
...
def _build_event_user_prompt(date: str, events: list[dict[str, Any]]) -> str:
    ...
    return (
        "あなたはPC作業ログの解析者です。以下のイベント一覧から、各イベントを短く要約してください。\n"
        "\n"
        "重要:\n"
        "- 出力は必ずJSONのみ（余計な説明文は一切禁止）\n"
        "- **active_ocr が主作業**。inactive_ocr は「参照情報」として補助的に使う\n"
...
def step_summarize_events(
    ...
    *,
        ...
        to_process.append(
            {
                ...
                "inactive_display_ocr_texts": row.get("inactive_display_ocr_texts") or [],
            }
        )
    ...
    while i < len(to_process):
        ...
        for ev in batch:
            inactive_join = " / ".join(
                _collapse_ws(s) for s in (ev.get("inactive_display_ocr_texts") or []) if str(s or "").strip()
            )
            if len(inactive_join) > max_chars:
                inactive_join = inactive_join[:max_chars].rstrip() + "…"
            pe = {
                ...
                "active_ocr": (str(ev.get("active_display_ocr_text") or "")[:max_chars]).strip(),
                "inactive_ocr": inactive_join,
            }
        ...
        if pending:
            for eid in list(pending):
                ...
                aocr = str(pe.get("active_ocr") or "").strip()
                iocr = str(pe.get("inactive_ocr") or "").strip()
                short = (aocr[:120] + "…") if len(aocr) > 120 else aocr
                hint = f"（参照: {iocr[:80]}…）" if iocr else ""
                got[eid] = {
                    "event_id": eid,
                    "ts": str(pe.get("ts") or ""),
                    "summary": f"{ctx} を確認（要約失敗・OCR抜粋: {short}）{hint}".strip(),
