...
def _build_daily_user_prompt(date: str, hours: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "- highlights: 箇条書きの短文で3つ掲載。「時間帯ごとの作業」ではなく、1日全体を通して繰り返し行われている作業テーマを統合し、主要な作業TOP3を抽出してください。手順:1. タイムラインや概要から作業テーマを抽出する2. 同じ目的の作業を統合する（クラスタリング）3. 1日全体での重要度を判断する4. 主要作業TOP3を出す\n"
        "\n"
        "必ずJSONのみを返してください。\n\n"
        "出力JSONの形式:\n"
        "{\n"
...
def _build_hour_enrich_prompt(
    ...
    return (
        ...
        "  - 1文目は「目的」、2文目は「1日の位置づけ/意味」になるように構成すること\n"
        "\n"
        "## 注意\n"
